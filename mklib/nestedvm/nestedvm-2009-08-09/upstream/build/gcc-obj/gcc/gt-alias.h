...
static void gt_ggc_ma_reg_known_value PARAMS ((void *));
static void
gt_ggc_ma_reg_known_value (x_p)
      void *x_p;
{
  size_t i;
  struct rtx_def ** const x = (struct rtx_def **)x_p;
  if (ggc_test_and_set_mark (x))
    for (i = 0; i < (reg_known_value_size); i++)
      gt_ggc_m_7rtx_def (x[i]);
}

static void gt_ggc_ma_alias_invariant PARAMS ((void *));
static void
gt_ggc_ma_alias_invariant (x_p)
      void *x_p;
{
  size_t i;
  struct rtx_def ** const x = (struct rtx_def **)x_p;
  if (ggc_test_and_set_mark (x))
    for (i = 0; i < (alias_invariant_size); i++)
      gt_ggc_m_7rtx_def (x[i]);
}
